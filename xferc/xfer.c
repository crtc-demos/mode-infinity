...
typedef struct _filelist {
  ...
  unsigned int load, exec, length, attrs ;
  int type ;
...
static void crccalc(unsigned char *buffer, int len, unsigned int *crc)
{
...
static bbc_status_t getcurdir(serial_h com, char *buffer, int size)
{
  serial_printf(com, "C") ;
...
static bbc_status_t getbootopt(serial_h com, char *buffer, int size)
{
  serial_printf(com, "B") ;
...
static bbc_status_t getfileinfo(serial_h com, filelist *file)
{
  ...
  if ( (result = bbc_readline(com, buffer, MAXLINELEN)) == BBC_OK ) {
    if ( sscanf(buffer, "%s %x %x %x %x %d", file->name,
...
static bool check_pcname(char *type, char *pcname, int size)
{
  FILE *pcfile ;
...
static int filespec(char *buffer, int size, bool wildcards, bool directories)
{
...
static bbc_status_t retrieve_crc(serial_h com, filelist *file)
{
  ...
  if ( (result = bbc_readline(com, buffer, MAXLINELEN)) == BBC_SYNC &&
       ...
       fhandle != 0 &&
       (result = bbc_readline(com, buffer, MAXLINELEN)) == BBC_OK &&
       sscanf(buffer, "%d", &bbccrc) == 1 ) {
    printf("BBC file %s has checksum %x\n", file->name, bbccrc) ;
  }
...
static bbc_status_t retrieve_disc(serial_h com)
{
  ...
  case 1600 * 256:
    ...
    break ;
  default:
    printf("Unrecognised disc format, drive %c has %d bytes\n", drivech, size) ;
  }  
  ...
  if ( size > 160 || (size > 80 && drivech >= '2') ) {
    printf("Disc %c has more tracks than allowed (%d)\n", drivech, size) ;
    return BBC_OK ;
  }
  ...
  for ( ntracks = 0 ; !interrupted(false) && !trackerr && ntracks < size ; ++ntracks ) {
    int remaining = BBCTRACKSIZE, crc = 0 ;

    serial_printf(com, "G%c\r%d\r", drivech, ntracks) ;
    do {
      int rbytes = remaining ;

      if ( (result = bbc_read(com, buffer, &rbytes)) == BBC_OK ) {
        if ( !trackerr &&
             (int)fwrite(buffer, sizeof(char), rbytes, image) != rbytes )
          trackerr = true ;
        remaining -= rbytes ;
        crccalc(buffer, rbytes, &crc) ;
        printf("\rRead %d tracks of %d", ntracks + 1, size) ;
        fflush(stdout) ;
      } else /* Error reading bytes */
        break ;
    } while ( remaining > 0 ) ;
  ...
  if ( keyboard_line(buffer, MAXLINELEN) != 0 ) {
    ...
    while ( (result = bbc_readline(com, buffer, MAXLINELEN)) == BBC_OK ) {
      if ( log ) {
        ...
        if ( (int)fwrite(buffer, sizeof(char), len, log) != len ||
             fputc('\n', log) == EOF )
          log = NULL ;
      }
      puts(buffer) ;
